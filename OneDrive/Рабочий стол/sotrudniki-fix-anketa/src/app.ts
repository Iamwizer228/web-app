...
async function getTotalPages(city: string, query: string): Promise<number> {
  const link = `https://www.avito.ru/${city}/rezume?cd=1&q=${query}`;
  ...
  const totalPages = await page.evaluate(() => {
      ...
      if (!pagination || !pagination.lastElementChild) {
          return 1; 
      }
      return Number(pagination.lastElementChild.textContent) || 1; 
  });
...
app.post("/submit", async (req: Request, res: Response): Promise<void> => {
  ...
  try {
      ...
      const totalPages = await fetchAvitoData(formData); 
      res.json({
...
async function fetchAvitoData(formData: FormData) {
  ...
  const progressBar = new SingleBar({
      format: 'Парсинг |{bar}| {percentage}% | Страница: {value}/{total}',
      hideCursor: true
  });

  
  const totalPages = await getTotalPages(city, query);
